...
while run:
    for f in pygame.event.get():
        if f.type == pygame.QUIT:
            run = False
        elif f.type == pygame.KEYDOWN:
            if game_over:
                score = 0
                finish = delay
                game_over = False
                last_respawn_time = pygame.time.get_ticks()  # возвращение миллисекунд
        elif f.type == pygame.MOUSEBUTTONDOWN:
            if f.button == pygame.BUTTON_LEFT:
                if not game_over and target_rect.collidepoint(f.pos):
                    score += 1
                    respawn_target()
                    last_respawn_time = pygame.time.get_ticks()
                    finish = delay / (base ** score) # формула уменьшение задержки окончание игры
    clock.tick(FPS)
    monitor.fill(red)
    ...
    if past > finish:
        ...
        h = H - H * past / finish # формула для уменьшения заднего прямоугольника
